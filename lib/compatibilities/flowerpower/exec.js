...
console.log(`flowerpower start - ${flowerPower}`);
    ...
    rp(options)
      .then(function (devicetypes) {
        ...
        Promise.map(devicetypes, function (devicetype) {
          ...
          .then(function () {
            if (toUpdate) {
              flowerPower.discoverById(function(flowerPowerInstance) {
                console.log(`flowerpower discovered - ${flowerPowerInstance}`);

                // you can be notified of disconnects
                flowerPowerInstance.on('disconnect', function() {
                  console.log(`flowerpower - disconnected! :(`);
                });

                // you'll need to call connect and set up
                flowerPowerInstance.connectAndSetUp(function(err) {
                  console.log(`flowerpower - connected!`);
                  if (err) {
                    ...
                    Promise.map(localDevicetypes, function (devicetype) {
                      sendDevicestate(flowerPowerInstance, devicetype);
                    })
                      .then(function () {
                        if (peripheral.state === 'connected') {
                          console.log(`flowerpower - disconnectDevice ${identifier}`);
                          flowerPowerInstance.disconnect();
                        }
                      });
                  }
                });
  ...
  function sendDevicestate(flowerPowerInstance, devicetype) {
    console.log(`flowerpower sending - ${flowerPowerInstance}`);
    var lastChanged = new Date(devicetype.lastChanged);
    ...
    if (!lastChanged || (now - lastChanged) >= delays[type]) {
      switch (type) {
        case 'airTemperature':
          flowerPowerInstance.readAirTemperature(function (error, temperature) {
        ...
        case 'soilTemperature':
          flowerPowerInstance.readSoilTemperature(function (error, temperature) {
        ...
        case 'light':
          flowerPowerInstance.readSunlight(function (error, sunlight) {
        ...
        case 'moisture':
          flowerPowerInstance.readSoilMoisture(function (error, soilMoisture) {
        ...
        case 'soilEC':
          flowerPowerInstance.readSoilElectricalConductivity(function (error, soilEC) {
        ...
        case 'batteryLevel':
          flowerPowerInstance.readBatteryLevel(function (error, batteryLevel) {
