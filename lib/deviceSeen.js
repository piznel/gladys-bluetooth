...
module.exports = function deviceSeen(peripheral) {
    ...
    if(shared.devices[identifier]) {
        ...
        if(shared.devices[identifier].hasOwnProperty('user') && shared.devices[identifier].user != null) {
            console.log(`Device "${peripheral.advertisement.localName}" is the peripheral of user ${shared.devices[identifier].user}, it means user is at home ID = ${config.gladysHouseId} ! `);
